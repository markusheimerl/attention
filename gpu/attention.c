...
// Initialize the attention network with configurable dimensions
...
__global__ void softmax_forward_kernel_attention(float* weights, float* scores, int batch_size, int seq_len) {
    ...
    if (batch_idx < batch_size && row_idx < seq_len) {
        float* batch_scores = scores + batch_idx * seq_len * seq_len + row_idx * seq_len;
        float* batch_weights = weights + batch_idx * seq_len * seq_len + row_idx * seq_len;
        
        // Find max for numerical stability
        float max_val = batch_scores[0];
        for (int j = 1; j < seq_len; j++) {
            if (batch_scores[j] > max_val) max_val = batch_scores[j];
        }
        
        // Compute softmax
        float sum = 0.0f;
        for (int j = 0; j < seq_len; j++) {
            batch_weights[j] = expf(batch_scores[j] - max_val);
            sum += batch_weights[j];
        }
        for (int j = 0; j < seq_len; j++) {
            batch_weights[j] /= sum;
...
__global__ void softmax_backward_kernel_attention(float* d_scores, float* d_weights, float* weights, int batch_size, int seq_len) {
    ...
    if (batch_idx < batch_size && row_idx < seq_len) {
        float* batch_weights = weights + batch_idx * seq_len * seq_len + row_idx * seq_len;
        float* batch_d_weights = d_weights + batch_idx * seq_len * seq_len + row_idx * seq_len;
        float* batch_d_scores = d_scores + batch_idx * seq_len * seq_len + row_idx * seq_len;
        
        // Softmax gradient: d_scores[i] = weights[i] * (d_weights[i] - sum_j(d_weights[j] * weights[j]))
        float sum = 0.0f;
        for (int j = 0; j < seq_len; j++) {
            sum += batch_d_weights[j] * batch_weights[j];
        }
        for (int j = 0; j < seq_len; j++) {
            batch_d_scores[j] = batch_weights[j] * (batch_d_weights[j] - sum);
        }
    }
}
...
void forward_pass_attention(Attention* attn, float* d_X) {
    ...
    for (int layer = 0; layer < attn->num_layers; layer++) {
        ...
        dim3 blocks(attn->batch_size);
        dim3 threads(attn->seq_len);
        softmax_forward_kernel_attention<<<blocks, threads>>>(
            attn->d_attn_weights[layer], attn->d_attn_scores[layer],
...
void backward_pass_attention(Attention* attn, float* d_X) {
    ...
    for (int layer = attn->num_layers - 1; layer >= 0; layer--) {
        ...
        // ∂L/∂W_o = attn_output^T * d_layer_output
        CHECK_CUBLAS(cublasSgemm(attn->cublas_handle,
        ...
        // ∂L/∂attn_output = W_o^T * d_layer_output
        CHECK_CUBLAS(cublasSgemm(attn->cublas_handle,
                                CUBLAS_OP_T, CUBLAS_OP_N,
                                attn->d_model, total_seq, attn->d_model,
                                &alpha, attn->d_W_o[layer], attn->d_model,
        ...
        for (int batch = 0; batch < attn->batch_size; batch++) {
            ...
            dim3 blocks(1);
            dim3 threads(attn->seq_len);
            softmax_backward_kernel_attention<<<blocks, threads>>>(
                d_attn_scores_batch, d_attn_weights_batch, attn_weights_batch,
                1, attn->seq_len
            );
            
            // ∂L/∂Q = K * d_scores^T / sqrt(d_model)
            float scale = 1.0f / sqrtf(attn->d_model);
            CHECK_CUBLAS(cublasSgemm(attn->cublas_handle,
                                    CUBLAS_OP_N, CUBLAS_OP_T,
                                    attn->d_model, attn->seq_len, attn->seq_len,
                                    &scale, K_batch, attn->d_model,
                                    d_attn_scores_batch, attn->seq_len,
                                    &beta, dQ_batch, attn->d_model));
            
            // ∂L/∂K = Q * d_scores / sqrt(d_model)
            CHECK_CUBLAS(cublasSgemm(attn->cublas_handle,
                                    CUBLAS_OP_N, CUBLAS_OP_N,
                                    attn->d_model, attn->seq_len, attn->seq_len,
        ...
        if (layer > 0) {
            // ∂L/∂input = W_q^T * dQ + W_k^T * dK + W_v^T * dV
            CHECK_CUBLAS(cublasSgemm(attn->cublas_handle,
                                    CUBLAS_OP_T, CUBLAS_OP_N,
                                    attn->d_model, total_seq, attn->d_model,
                                    &alpha, attn->d_W_q[layer], attn->d_model,
...
void save_attention(Attention* attn, const char* filename) {
    ...
    for (int layer = 0; layer < attn->num_layers; layer++) {
        ...
        CHECK_CUDA(cudaMemcpy(W_q, attn->d_W_q[layer], weight_size * sizeof(float), cudaMemcpyDeviceToHost));
        CHECK_CUDA(cudaMemcpy(W_k, attn->d_W_k[layer], weight_size * sizeof(float), cudaMemcpyDeviceToHost));
        CHECK_CUDA(cudaMemcpy(W_v, attn->d_W_v[layer], weight_size * sizeof(float), cudaMemcpyDeviceToHost));
        CHECK_CUDA(cudaMemcpy(W_o, attn->d_W_o[layer], weight_size * sizeof(float), cudaMemcpyDeviceToHost));
        
        fwrite(W_q, sizeof(float), weight_size, file);
        fwrite(W_k, sizeof(float), weight_size, file);
        fwrite(W_v, sizeof(float), weight_size, file);
        fwrite(W_o, sizeof(float), weight_size, file);
...
Attention* load_attention(const char* filename, int custom_batch_size, cublasHandle_t cublas_handle) {
    ...
    for (int layer = 0; layer < num_layers; layer++) {
        ...
        fread(W_q, sizeof(float), weight_size, file);
        fread(W_k, sizeof(float), weight_size, file);
        fread(W_v, sizeof(float), weight_size, file);
        fread(W_o, sizeof(float), weight_size, file);

...
typedef struct {
    ...
    // Adam parameters
    float* d_W_q_m; float* d_W_q_v;
    float* d_W_k_m; float* d_W_k_v;
    float* d_W_v_m; float* d_W_v_v;
    float* d_W_o_m; float* d_W_o_v;
    float beta1;
    float beta2;
    float epsilon;
    int   t;
    ...
    // scores/weights are multi-head: [num_heads * batch_size x seq_len x seq_len]
    half* d_scores;
    half* d_attn_weights;
    ...
    half* d_grad_weights;     // [num_heads * batch_size x seq_len x seq_len]
    half* d_grad_scores;      // [num_heads * batch_size x seq_len x seq_len]
    ...
    cublasLtMatrixLayout_t seq_batch_layout; // [seq_len x d_model] batched (unused in MH)
    cublasLtMatrixLayout_t attn_batch_layout;// [seq_len x seq_len] batched (per-head, over batch)
    cublasLtMatrixLayout_t head_seq_layout;  // [seq_len x head_dim] batched over batch_size
